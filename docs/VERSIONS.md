...
| v0.3.0 | GitHub 协作闭环 | 验证结果停留在本地，无法融入研发协作 | Issue 自动任务、GitHub Check、Issue 报告、Actions Artifact | 当前版本 |
| v0.4.0 | 页面质量分析 | 现有检查维度有限，难以覆盖完整页面质量 | 可访问性、性能和页面质量报告 | 规划中 |
...
- 集成 axe-core，输出更完整的 WCAG 可访问性问题。
- 采集 Core Web Vitals 和关键加载指标。
- 增加按页面、设备和问题类型划分的质量趋势。
- 支持质量阈值和 GitHub Check 阻断规则。
- 为问题提供更具体的 DOM 定位信息和修复建议。

该版本尚未进入开发，最终范围以正式需求评审为准。
